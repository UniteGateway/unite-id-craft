...
export interface CardTemplate {
  id: string;
  name: string;
  image: string; // data URI of the template
}
...
const TemplateManager: React.FC = () => {
  ...
  const handleAIGenerate = async () => {
    ...
    toast.info("AI template generation coming soon! Your prompt: " + aiPrompt.slice(0, 50));
        ...
        <div className="rounded-xl border border-border bg-card p-4 space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Upload complete card template images. Supported: PNG, JPG, WEBP
            </p>
            ...
            <div className="flex items-center gap-3">
              ...
              {customLogo && (
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => { setCustomLogo(null); toast.info("Logo removed"); }}
                >
                  <X className="h-4 w-4" />
                </Button>
              )}
